...
(function(){
	...
	function removePath(filename,isdir){
		var c=confirm(_('Confirm delete')+' '+filename+' ?');
		if (c){
			iwxhr.get('/cgi-bin/luci/admin/nas/fileassistant/delete',
			{path:concatPath(currentPath,filename),isdir:isdir},
			function (x,res){
				if (res.ec === 0){refresh_list(res.data,currentPath);}});
		}
	}

	function installPath(filename,isdir){
		if (isdir === "1"){
			alert(_('This is a directory,please select the ipk file to install!'));
			return;
		}

		var isipk=isIPK(filename);
		if (isipk === 0){
			alert(_('Only ipk files are allowed to be installed!'));
			return;
		}

		var c=confirm(_('Confirm install')+' '+filename+' ?');
		if (c){iwxhr.get('/cgi-bin/luci/admin/nas/fileassistant/install',
			{filepath:concatPath(currentPath,filename),isdir:isdir},
			function (x,res){
				if (res.ec === 0){
					location.reload();
					alert(_('Install successful!'));
				}else{
					alert(_('Install failed,please check the file format!'));
				}
			});
		}
	}
	...
	function renamePath(filename){
		var newname=prompt(_('Please enter a new file name:'),filename);
		if (newname){
	...
	function openpath(filename,dirname){
		...
		window.open('/cgi-bin/luci/admin/nas/fileassistant/open?path='+encodeURIComponent(dirname)+'&filename='+encodeURIComponent(filename));
	}
		...
		if (filenames){
			for (var i=0;i < filenames.length;i++){
				...
				if (line){
					...
					+'<strong>'+o.displayname+'</strong>'+"&nbsp&nbsp"+'</td>'
					+'<td class="cbi-value-field cbi-value-owner">'+o.owner+"&nbsp&nbsp"+'</td>'
					+'<td class="cbi-value-field cbi-value-date">'+o.date+"&nbsp&nbsp"+'</td>'
					+'<td class="cbi-value-field cbi-value-size">'+o.size+"&nbsp&nbsp"+'</td>'
					+'<td class="cbi-value-field cbi-value-perm">'+o.perms+"&nbsp&nbsp"+'</td>'
					+'<td class="cbi-section-table-cell">\
					<button class="cbi-button cbi-button-edit">'+_('Rename')+'</button>\
					<button class="cbi-button cbi-button-remove">'+_('Delete')+'</button> '+install_btn+'</td>'+'</tr>';
				}
			}
		}
		listHtml+="</table>";
		...
		if (!fullPath){
			...
			xhr.onload=function(){
				if (xhr.status == 200){
					...
					alert(_('Upload failed,please try again later...'));
				}
			};
			xhr.send(formData);
		}
